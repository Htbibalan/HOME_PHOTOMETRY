...
class FED3MonitorApp:
    ...
    def show_port_mapping_message(self):
        message = (
            "1) Poke or trigger the pellet sensor on FED3 units to identify the assigned ports.\n"
            "2) If you need to restart a FED3 after pressing START, do it one at a time.\n"
            "3) Ports are assigned based on the initial detection when the program started.\n"
            "4) GPIO pin mapping for Raspberry Pi 4B is:\n\n"
    ...
    def create_layout(self):
        ...
        self.start_button.grid(column=0, row=6, columnspan=2, sticky="we", pady=10)
        ...
        self.stop_button.grid(column=0, row=7, columnspan=2, sticky="we", pady=10)
        self.canvas = tk.Canvas(controls_frame, width=120, height=120)
        self.canvas.grid(column=0, row=8, columnspan=2, pady=20)
    ...
    def display_recording_indicator(self):
        ...
        if self.session_timer_text is None:
            self.session_timer_text = self.canvas.create_text(40, 90, text="Session Time: 00:00:00", font=("Cascadia Code", 10), anchor="n")
